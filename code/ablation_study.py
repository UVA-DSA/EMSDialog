    ...
    #     print(f"load {model_name_or_path} successfully, think={think}")
        ...
        if "Qwen3" in model_name_or_path:
            ...
            model = LLM(
                ...
                tensor_parallel_size=tp,         # >1 if using multiple GPUs
                trust_remote_code=True,         # Qwen needs this
                gpu_memory_utilization=0.97,    # let vLLM use most of the GPU RAM
                # max_model_len=8192, # if using 2-a100
                max_model_len=4096, # if using 1-h200
            )
            VLLM_DEFAULT_PARAMS = SamplingParams(
                temperature=1.0,
                top_p=0.75,
...
def generate_dialogue(
    ...
    for j in range(critic_passes):
        ...
        if not any([fact_critique, tf_critique, style_critique]):
            print(f"[generate_dialogue] critics produced no actionable feedback at pass {j}; stopping.")
            break
    
        # merge failing critiques
        merged = "\n".join(s for ok, s in [
            (fact_ok,  fact_critique),
            (tf_ok,    tf_critique),
            # (style_ok, style_critique),
        ] if not ok and s).strip()
...
def refine_dialogue(
    ...
    """
    Simple dialogue refinement WITHOUT critics.

    Pipeline:
      - normalize & parse the initial dialogue
      - iteratively ask the LLM to refine it (up to n_pass times)
      - stop early if the revision is empty or unchanged
    ...
    for i in range(n_pass):
        ...
        revised, _ = call_llm(
            ...
            role="refiner_reviser",   # same role as before, just without critiques
            max_regen=max_regen,
            sampling=sampling,
            max_token=max_token,
        )
        ...
        if not new_text_raw:
            print(f"[refine_dialogue] refiner_reviser returned empty at pass {i}; stopping.")
            break
...
#         if fact_ok and tf_ok and style_ok:
...
#             (fact_ok,  fact_critique),
#             (tf_ok,    tf_critique),
#             # (style_ok, style_critique),  # optional: add if you want style to drive revisions too
#         ] if not ok and s).strip()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enable_refine", action="store_true", help="if use refine in Qwen3")
    parser.add_argument("--enable_checker", action="store_true", help="whether to use checker in Qwen3")
    parser.add_argument("--enable_plan", action="store_true", help="whether to use plan in Qwen3")
    ...
    if args.enable_refine:
        desc = "Generate → Refine"
    
    if args.enable_plan and args.enable_refine:
        desc = "Plan → Generate → Refine"
    ...
    log_dir = f"../log/{model_name_or_path}/{desc}"
        ...
        if args.enable_plan:
            if "final_plan.txt" not in os.listdir(case_dir):
                plan_text = generate_plan(
                    ...
                    critique_passes=critique_passes,
